...
public class Caesar : Helper
{
	...
	public static string caesarDecrypt(string cipherText)
	{
		...
		{ //find the index of the smallest chi statisic
...
[ublic class Vigenere : Helper
{
	...
	static int estimateKeyLength(string cipherText)
	{
		...
		for (int keyLength = 1; keyLength <= 10; keyLength++)
		{
			...
			{ //the first key length which has margin of 0.25 from 1.73 is the estimation 
				len = keyLength;
				break;
			}
		}
	...
	static string[] split(string plainText, int keyLength)
	{
		...
		{ //go for every character in the text
	...
	{ //reverse to the split method
		...
		while (true)
		{
			...
			{
				while (true)
				{ //find the next part which is not empty
					index++;
	...
	static public string Solve(string input)
	{
		...
		return "Vigenere Attempt Complete";
	}	
}
